...
bool GA::onCreate(int a_argc, char* a_argv[]) 
{
	...
	for (int i = 0; i < 100; i++)
	{
		...
		{
			int  upperpits = rand();
			agent.genome[j] = (upperpits << 16) + rand();
		}
...
void GA::evolve()
{
	int populationSize = m_population.size();
...
void GA::crossover(Agent & a_agent1, Agent & a_agent2)
{
	if (unitRand()<m_crossoverRate)
	{
		...
		for (; startBit < BIT_COUNT; startBit++)
		{
			...
			int chromsome = startBit / 32;

			//which bit?

			int bit = startBit % 32;

			int temp1 = a_agent1.genome[chromsome] & (1 << bit);
			int temp2 = a_agent2.genome[chromsome] & (1 << bit);

			if (temp1 == 0)
				a_agent2.genome[chromsome] &= (1 << bit);
			else
				a_agent2.genome[chromsome] |= (1 << bit);


			if (temp2 == 0)
				a_agent1.genome[chromsome] &= (1 << bit);
			else
				a_agent1.genome[chromsome] |= (1 << bit);
...
void GA::mutate(Agent& a_agent)
{
	for (int startBit = 0; startBit < BIT_COUNT; startBit++)
	{
		...
		{
			a_agent.genome[chromsome] ^= (1 << bit);
		}
	}
